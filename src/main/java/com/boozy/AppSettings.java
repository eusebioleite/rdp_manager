...
public class AppSettings {
    ...
    private static String jdbc_string = 
        String.format(
            "jdbc:sqlite:%s\\rdp_manager\\rdp_manager.db", System.getProperty("user.home")
        );
    ...
    public static void create_db() {
        ...
        new File(System.getProperty("user.home") + "/rdp_manager").mkdirs();
        ...
        try(Connection connection = DriverManager.getConnection(jdbc_string)) {
            ...
            /* credentials */
            statement.execute("CREATE TABLE IF NOT EXISTS credentials (id INTEGER PRIMARY KEY AUTOINCREMENT, username varchar, password varchar, rdp_id INTEGER, CONSTRAINT credentials_FK_1 FOREIGN KEY (rdp_id) REFERENCES rdp(id))");


            /* insert default values, but check if exists before insert */
            /* types */
    ...
    public static void delete_types(Integer id){

        try(Connection connection = DriverManager.getConnection(jdbc_string)) {
            
            /* update types query */
            Statement statement = connection.createStatement();
            statement.execute(String.format("delete from typés where types.id = %d", id));
